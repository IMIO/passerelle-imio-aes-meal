pipeline {
    ...
    stages {
        stage('Build') {
            ...
            steps {
                sh "fpm -a amd64 --python-bin python3 --python-pip pip3 --python-package-name-prefix python3 -n passerelle-imio-aes-meal -s python -t deb -v `echo ${VERSION}` --python-install-lib /usr/lib/python3/dist-packages -d passerelle setup.py"
